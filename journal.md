...
# RestControllerAdvice/Global Exception Handling, ErrorResponse DTO

Learned about `@RestControllerAdvice` and how to handle exceptions in one place to avoid
repetitive code with a `GlobalExceptionHandler` class. Created custom exception classes
(`EntityNotFoundException` and
`BadRequestException`) that can be thrown directly in the service layer if errors occur. This
improves the code by handling exceptions closer to where they happen rather than letting them
potentially bubble up and clean up controller methods. Instead of individual try/catch blocks, each
controller will send exceptions to the global exception handler where it will send an 
`ErrorResponse` record in the exact format specified. Applied the changes to the Aircraft and 
Pilot to include updating tests first to expect the new exception types.
